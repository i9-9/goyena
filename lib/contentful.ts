...
function parseProjectEntries(entries: any): Project[] {
  if (!entries || !entries.items) return [];
  
  return entries.items.map((item: any) => parseProjectEntry(item));
}
...
function parseProjectEntry(entry: any): Project {
  ...
  const images = fields.images?.map((img: any) => ({
...
export async function getProjectBySlug(slug: string): Promise<Project | null> {
  try {
    ...
    const projects = parseProjectEntries(entries);
